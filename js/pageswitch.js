(function($){
	...
	var SP = $.fn.switchPage = function(options){//$.fn即在原型上操作
		...
		return this.each(function(){//each是指循环作用
	...
	//重写鼠标滑动事件
	$(document).on("mousewheel DOMMouseScroll", MouseWheelHandler);//浏览器兼容 
	function MouseWheelHandler(e) {
		e.preventDefault();
		var value = e.originalEvent.wheelDelta || -e.originalEvent.detail;
		//Firefox滚轮方向与别的相反，所以添加负号表示一致  
	...
	function initEffects(dest,element){
		...
		canScroll = false;//避免一次滚动造成的多次滑动，添加一个锁
		if(isSuportCss(transform) && isSuportCss(transition)){//判断兼容性
			var traslate = "";
			if(opts.direction == "horizontal"){
				traslate = "-"+dest.left+"px, 0px, 0px";//横滑获取left
			}else{
				traslate = "0px, -"+dest.top+"px, 0px";//竖滑
			}
			container.css({
	...
	win.resize(function(){
		...
		resizeId = setTimeout(function(){
			reBuild();
		},500);//定义一个延时是为了避免窗口大小改变时频繁调用reBuild
	});

	function reBuild(){//窗口大小变化时重新调整大小，重新定位
		var currentHeight = win.height(),
			currentWidth = win.width();

		var element = arrElement[iIndex];
		if(opts.direction == "horizontal"){//横滑
			var offsetLeft = element.offset().left;
			if(Math.abs(offsetLeft)>currentWidth/2 && iIndex <(arrElement.length-1)){
				iIndex ++;
			}
		}else{//竖滑
			var offsetTop = element.offset().top;//当前对象针对文档的TOP值
